'use strict';
// Figura dibujada: la hoja actual aún no contiene un paso con apoyos estables.
// Elegir cuadros por distancia sincroniza el ritmo, pero no corrige sus poses.
...
const bg = new Image(); bg.src = 'mountain.webp';
...
function frame(ts) {
  ...
  // Reservar el ancho completo del dibujo, también en los extremos del intento.
  const margen = mobile ? .06 : .04;
  const izq = clamp((W * margen + unidad * .93 - ox) / bw, .02, .9);
  const der = clamp((W * (1 - margen) - unidad * 1.12 - ox) / bw, izq + .1, .98);
  ...
  // Sin proyección inventada del sprite: el contacto se evalúa sin sombra añadida.
  ...
  if (advancing && !SOLO_SUBIDA && fase !== 3 && Math.random() < dt * (fase === 2 ? 40 : 7)) {
  ...
  // El grabado no se vuelve a dibujar encima de los pies ni de la piedra.
